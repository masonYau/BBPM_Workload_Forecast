...
class DataProcessor:
    ...
    def calculate_remaining_bow_volume(self, cutoff_date=None):
        ...
        self.calculate_received_volume(cutoff_date=cutoff_date)

        input_frequency = self.input_bow_volume_config["frequency"]
        cutoff_input_period = cutoff_date.to_period(input_frequency)
        ...
        for (case_type, input_period), planned_volume in self.input_bow_volume.sort_index().items():
            ...
            if input_end_date <= cutoff_date:
                continue

            allocation_start_date = max(input_start_date, cutoff_date)
            allocation_volume = planned_volume

            if input_period == cutoff_input_period:
                received_volume = received_by_input_period.get((case_type, input_period), 0)
                allocation_volume = max(planned_volume - received_volume, 0)
        ...
        periods = self.bow_volume.index.union(self.received_volume.index).union(remaining_output_volume.index).sort_values()
